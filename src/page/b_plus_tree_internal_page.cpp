...
//Copy this to recipient
   int B=recipient->GetSize();
    for(int i=0;i<GetSize();i++)
    {
      recipient->array_[B+i]=array_[i];
    }
                                                       ...
                                                       BufferPoolManager *buffer_pool_manager)
{
    ...
    recipient->CopyFirstFrom(finally,buffer_pool_manager);
    //Change the parent.
    ...
    ToChangePage->SetParentPageId(recipient->GetParentPageId());
    buffer_pool_manager->UnpinPage(ToChangePage->GetPageId(),true);
    buffer_pool_manager->UnpinPage(this->GetPageId(),true);
    buffer_pool_manager->UnpinPage(recipient->GetPageId(),true);
}
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::CopyFirstFrom(const MappingType &pair, BufferPoolManager *buffer_pool_manager)
{
  //Fetch the parent
    ...
    int ind_in_par=MyParent->ValueIndex(GetPageId());
    auto par_key=MyParent->KeyAt(ind_in_par);
    //Update the parent node.
   MyParent->SetKeyAt(ind_in_par,pair.first);
