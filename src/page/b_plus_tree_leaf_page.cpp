...
int B_PLUS_TREE_LEAF_PAGE_TYPE::RemoveAndDeleteRecord(const KeyType &key, const KeyComparator &comparator,BufferPoolManager*buffer_pool_manager)
{
   ...
   if(i<GetSize())
   {
      if(comparator(GetItem(i).first,key)==0) //the key exactly exists in the leaf page.
      {
        ...
        IncreaseSize(-1);
           //if i==0 and it's not root, we need to update all the parent's node's index.
        //Change key into newkey
        
         if(i==0&& !IsRootPage())
         {
             KeyType newkey=array_[0].first;
             page_id_t par_id = GetParentPageId();
             while(par_id!=INVALID_PAGE_ID)
             //parent page VALID
             {
                 Page*p=buffer_pool_manager->FetchPage(par_id);
               //Get parent page
               BPlusTreeInternalPage<KeyType,page_id_t,KeyComparator>*parent=reinterpret_cast<BPlusTreeInternalPage<KeyType,page_id_t,KeyComparator>*>(p->GetData());
               //binary search key 
                int keyindex=parent->KeyIndex(key,comparator);
                if(keyindex>0 && keyindex < parent->GetSize())//FoundKey
                {
                    parent->SetKeyAt(keyindex,newkey);
                    //Update keys in parent.
                    buffer_pool_manager->UnpinPage(parent->GetPageId(),true);
           
                    break;
                }
                //keyindex==-1,Not found.
                 par_id=parent->GetParentPageId();
...
{
    //Assume "recipient" page is prior of this page
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void B_PLUS_TREE_LEAF_PAGE_TYPE::MoveLastToFrontOf(BPlusTreeLeafPage *recipient,BufferPoolManager *bufferpoolmanager,int index)
{
     ...
     IncreaseSize(-1);//Size--
     recipient->CopyFirstFrom(finalElement,bufferpoolmanager,index);
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
void B_PLUS_TREE_LEAF_PAGE_TYPE::CopyFirstFrom(const MappingType &item,BufferPoolManager *bufferpoolmanager,int index) 
{
  ...
  for(int i=GetSize()-1;i>=0;i--)
   ...
   ParPage->SetKeyAt(index, item.first);
   //Change parent page
   bufferpoolmanager->UnpinPage(GetParentPageId(),true);  
 }
